...
class ImpressaoController extends FapescController {
    ...
    public function impressaoAction($idRelatorio) {
        return array_merge(
                        $this->usuario(), $this->menu("relatorio", "impressao", $idRelatorio), $this->info($this->findRelatorio($idRelatorio)->getProjeto()->getId(), $idRelatorio), array("idRelatorio" => $idRelatorio)
        );
    }
    ...
    public function imprimirAction($idRelatorio) {
        ...
        $arquivo = $relatorio->getProjeto()->getTitulo() . "-" . $relatorio->getNota() . "-" . $relatorio->getLiberacao("dmY") . "-" . $relatorio->getRubricas($relatorio->getRubrica);
