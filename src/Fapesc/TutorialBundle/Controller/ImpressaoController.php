...
class ImpressaoController extends FapescController {
    ...
    public function impressaoAction($idRelatorio) {
        ...
        return array_merge(
            $this->usuario(),
            $this->menu("relatorio", "impressao", $idRelatorio),
            $this->info($this->getDoctrine()->getRepository("FapescTutorialBundle:Relatorio")->find($idRelatorio)->getProjeto()->getId(), $idRelatorio),
            $dados
        );
    }
    ...
    public function imprimirAction($idRelatorio) {
        ...
        $arquivo = $projeto->getTitulo() . "-" . $relatorio->getNota() . "-" . $relatorio->getLiberacao("dmY") . "-" . $relatorio->getRubrica(true);
        return new Response($mpdf->Output($arquivo, 'D')); //D: download; I: direto na página
    }
...
//				"Caro Coordenador, substitua esta folha pelo demonstrativo de rendimentos relativo à conta bancária no. ". $projeto->getConta() ." / agência ". $projeto->getAgencia() . " - Banco " . $projeto->getBanco() . ". Caso a cópia impressa do extrato já esteja no formato A4, apenas substitua esta folha pelo referido documento. Caso não existam rendimentos a serem declarados, apenas descarte esta folha.",
        ...
        /*         * ******************************************************************************************************************************************************** */
        /*         * ******************************************************************************************************************************************************** */
        /* $itens = $this->getDoctrine()->getEntityManager()->getRepository("FapescTutorialBundle:Relatorio")->findBy(array("relatorio" => $idRelatorio));
          if (!empty($itens)) {
          ...
          if($dispendio->getCategoria() == 99) {//diarias categoria nao definida
          $dados = array();
          }
          else{
          $dados = array(
          "fornecedor"=>$dispendio->getFornecedor()->getNome(),
          "cnpj"=> $dispendio->getFornecedor()->getCadastro(),
          "distribuicao" => $dispendio->getDescricao(),
          "documentoFiscal" => $dispendio->getDocumento(),//TODO confirma
          "serie" => $dispendio->getUnidade(),//TODO confirmar
          "dataEmissao" => $dispendio->getData(),
          "valor" => $dispendio->getValor(),
          "noChequeRecibo" => $dispendio->getComprovante(),//TODO confirmar
          "dataPagamento" => "#definir",//TODO perguntar
          );
          $paginas[] = array(0 => $this->escreve("02-planoAplicacao",$dados) , false);
          $paginas[] = array(0 => $this->escreve("02-planoAplicacao",$dados) , false);
          }
        ...
        if (!$vazio) {
            ...
            for ($i = 1; $i <= 8; $i++) {
                ...
                if ($qtdDados != 0) {
                    ...
                    for ($j = 1; $j <= $qtdDados; $j++)
                        $my_array[$i][$j - 1][0] = "$contaTit" . "." . "$j";
                    $contaTit++;
                    $footer[1] = ($i == 8) ? $subTit[2] : $subTit[2] - 1;
                    $header[1] = $subTit[2];
    ...
    function geraPlanoAplicacao($idRelatorio) {
        ...
        $k = $titulos[$i];
        $titulo = $capitalAtivo ? "CAPITAL" : "CUSTEIO";
        $contaSub = 1;
        $tabelas = "<p><h4>" . "$titulo" . "</h4></p>"; //titulo
        for ($j = 1; $j <= $tam; $j++) {
            ...
            if ($contDados != 0) {
                ...
                $subTitulo .= "</h4>";
                for ($n = 1; $n <= $contDados; $n++)
                    $my_array[$i][$j][$n - 1][0] = "$contaSub" . "." . "$n";
                $contaSub++;
                $tabelas .= $subTitulo;
                $footer[1] = ($j == 5) ? $colunas[2] : $colunas[2] - 1;
                $header[1] = $colunas[2];
                $vtotal = $total[$i][$j];
                ...
                switch ($item->getCategoria()) {
                    case "1": //dispendio
                        ...
                        if (is_object($dispendio)) {
                            ...
                            switch ($dispendio->getCategoria()) {
                                case "1": //bibliografia
                                    $dados = array(1 => $dispendio->getDescricao(), $dispendio->getQuantidade(), $dispendio->getUnitario(), "R$ " . $dispendio->getTotal());
                                    $k = 3;
                                    $i = 1;
                                    $j = 3;
                                    break;
                                ...
                                case "3": //mobiliario
                                    ...
                                    break;
                                case "4": //pessoaFisica
                                    $dados = array(1 => $dispendio->getDescricao(), "R$ " . $dispendio->getTotal());
                                    $k = 5;
                                    $i = 2;
                                    $j = 2;
                                    break;
                    ...
                    case "2": //passagem
                        ...
                        if (is_object($passagem)) {
                            ...
                            $dados = array(1 => $passagem["descricao"], $saida, $chegada, "dataPag", "R$ " . $passagem["total"]);
                            $k = 6;
                            $i = 2;
                            $j = 4;
                        }
                    ...
                    case "3": //bolsa
                        ...
                        if (is_object($bolsa)) {
                            ...
                            $dados = array(1 => $bolsa["bolsista"]["nome"], $periodo, $bolsa["data"], "R$ " . $bolsa["valor"]);
                            $k = 8;
                            $i = 2;
                            $j = 5;
                        }
    ...
    function geraClass($pagina) {
        ...
        if ($pagina[0] == "planoAplicacao" || $pagina[0] == "contrapartida") {
            switch ($pagina[1]) {
                case 5:
                    $class = array(0 => "numero", "itens", "qtd", "vUnit", "vTotal");
                    break;
                case 3:
                    $class = array(0 => "numero", "itens", "vTotal");
                    break;
                case 4:
                    $class = array(0 => "numero", "bolsista", "periodo", "data", "vTotal");
                    break;
                case 6:
                    $class = array(0 => "numero", "trecho", "data", "data", "data", "vTotal");
                    break;
            }
        }
        return $class;
