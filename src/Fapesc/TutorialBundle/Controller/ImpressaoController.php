...
class ImpressaoController extends FapescController {
    ...
    private function criaPaginas($idRelatorio) {
        ...
        $dados = array(//relatorio1
            ...
            "partipacaoCusteio" => "R$ " . $contrapartida[0],
            "partipacaoCapital" => "R$ " . $empenho[0],
    ...
    function paginasDispendio($idRelatorio, $pagina) {//$pagina = "Contrapartida" ou "Empenho"
            ...
            foreach ($itens as $item) {
                ...
                switch ($item->getCategoria()) {
                    case "1": //dispendio
                        $dispendio = $this->getDoctrine()->getEntityManager()->getRepository("FapescTutorialBundle:Dispendio")->find($item->getItem());
                        if (is_object($dispendio)) {
                            $dados = array($dispendio->getData(), $dispendio);
                            $categoria = $dispendio->getCategoria();
                            if ($categoria == 6)
                                $dados[] = 1; //precisa de anexos extras
                            else
                                $dados[] = 0; //nao precisa de anexos extras
                        }
                        break;
                    case "2": //passagem
                        $passagem = $this->getDoctrine()->getEntityManager()->getRepository("FapescTutorialBundle:Passagem")->find($item->getItem());
                        if (is_object($passagem)) {
                            $passagemArray = $passagem->toArray();
                            $dados = array($passagemArray["compra"], $passagem, 2);
                        }
                        break;
                    case "3": //bolsa
                        $bolsa = $this->getDoctrine()->getEntityManager()->getRepository("FapescTutorialBundle:Bolsa")->find($item->getItem());
                        if (is_object($bolsa)) {
                            $bolsaArray = $bolsa->toArray();
                            $dados = array($bolsaArray["data"], $bolsa, 3);
                        }
                        break;
                }//fim switch
                if (isset($dados))
        ...
        foreach ($my_array as $item) {
            if ($item[2] == 0) {//nao precisa de anexos extras
                ...
                switch ($item[2]) {
                    case "1"://diaria
                        ...
                        $dados = array(
                            ...
                            "valor" => $diaria["tipo"] == 1 ? "R$ 110,00" : "R$ 55,00",
